...
__INLINE uint32_t co_wrap_add(uint32_t augend, uint32_t addend, uint32_t max)
{
    if (max - addend <= augend) {
        augend -= max - addend;
    } else {
        augend += addend;
    }
...
__INLINE uint32_t co_wrap_substract(uint32_t minuend, uint32_t subtrahend, uint32_t max)
{
    if (minuend >= subtrahend) {
        minuend -= subtrahend;
    } else {
        minuend += max - subtrahend;
    }

    return minuend;
}

__INLINE uint16_t co_wrap_substract_16(uint16_t minuend, uint16_t subtrahend, uint16_t max)
{
    if (minuend >= subtrahend) {
        minuend -= subtrahend;
    } else {
